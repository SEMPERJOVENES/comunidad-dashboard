...
import { Store, Plus, ShoppingBag, Loader2, Minus, Trash2, Check } from 'lucide-react';
...
export default function VentasPresencialesPage() {
  ...
  const [items, setItems] = useState<{ productId: number; productTitle: string; quantity: number; unitPrice: number }[]>([]);
  ...
  function addProduct(product: ShopifyProduct) {
    ...
    if (existing) {
      ...
      setItems([...items, { productId: product.id, productTitle: product.title, quantity: 1, unitPrice: price }]);
    }
  }
      ...
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500">Registra ventas en persona (Bizum, efectivo, transferencia)</p>
            </div>
          </div>
          <button
        ...
        {showForm && (
          <Card>
            ...
            {/* Product selector */}
            <div className="mb-4">
              <label className="block text-xs font-medium text-gray-500 mb-1">Productos (del catálogo Shopify)</label>
              <div className="flex flex-wrap gap-2 max-h-32 overflow-y-auto border border-gray-100 rounded-lg p-2">
                {products.map(p => (
                  <button
                    key={p.id}
                    onClick={() => addProduct(p)}
                    className="text-xs px-2 py-1 bg-gray-100 hover:bg-violet-100 text-gray-700 rounded-md transition-colors truncate max-w-[200px]"
                  >
                    + {p.title}
                  </button>
                ))}
            ...
            {items.length > 0 && (
              <div className="mb-4 space-y-2">
                <label className="block text-xs font-medium text-gray-500">Artículos seleccionados</label>
                {items.map(item => (
