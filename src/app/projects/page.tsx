...
import { FolderKanban, Plus, TrendingUp, ShoppingCart } from 'lucide-react';

export default function ProjectsPage() {
  const ranges = getDateRanges();
  const [selectedRange, setSelectedRange] = useState<DateRange>(ranges[3]);

  const totalRevenue = demoProjects.reduce((sum, p) => sum + p.totalRevenue, 0);
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <Card>
            ...
            <p className="text-2xl font-bold text-gray-900 mt-1">{demoProjects.length}</p>
          </Card>
          <Card>
            <p className="text-xs text-gray-500 font-medium">Órdenes Totales</p>
            <p className="text-2xl font-bold text-gray-900 mt-1">{formatNumber(demoProjects.reduce((s, p) => s + p.totalOrders, 0))}</p>
          </Card>
        </div>
